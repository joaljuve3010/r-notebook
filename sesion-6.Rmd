...
```
en el select usar : para establecer otros filtros..
...
counties %>%
  select(state, county, poberty_rate = poverty)
```

using transmute
```{r}
counties %>%
  transmute(state,county, population, density=populatio/land_area) %>%
  filter (population >=1000000 %>%
          arrange (desc(demsotu)))
```

choosing among the four verbes
```{r}
counties %>%
...
counties %>%
  select (state,county, population, contains("poverty"))
